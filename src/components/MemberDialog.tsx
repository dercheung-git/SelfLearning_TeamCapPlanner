...
export function MemberDialog({ onConfirm, onCancel }: MemberDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const capFloat = parseFloat(capacity);
    if (isNaN(capFloat) || capFloat <= 0) {
      setError("Capacity must be greater than 0");
      return;
    }
    onConfirm(name.trim(), capFloat);
  };
      ...
      >
        <div className="bg-slate-50 p-6 border-b-4 border-slate-900 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-100 rounded-lg border-2 border-slate-900">
               <UserPlus className="w-5 h-5 text-blue-600" />
            </div>
            <div>
              <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">New Member</h3>
              <p className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-widest mt-0.5">Registration Module</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Total Capacity (Days)</label>
              <div className="relative">
                <input
                  ...
                  placeholder="20.0"
                />
                <span className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-300 font-mono text-sm uppercase font-black">D</span>
              </div>
            </div>
          </div>
